...
export default function Categories() {
  ...
  useEffect(() => {
    api
      .get<{ products: Array<{ category: string; image?: string | null }> }>(
        '/api/products',
        { params: { limit: 100 } }
      )
      .then((res) => {
        const allProducts = (res as unknown as { products: Array<{ category: string; image?: string | null }> }).products || [];
        ...
        allProducts.forEach((p) => {
          ...
          map[cat].count += 1;
          if (!map[cat].image && p.image) map[cat].image = p.image;
        });

        const list: Category[] = Object.entries(map).map(([name, data]) => ({
          name,
          count: data.count,
