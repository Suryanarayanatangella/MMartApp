...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        const ordersRes  = await api.get('/api/orders/admin/all', { params: { limit: 100 } });
        const orders     = (ordersRes as unknown as { orders: Order[] }).orders || [];
        ...
        setStats({
          ...
          totalRevenue,
          pendingOrders,
          totalProducts: (productsRes as unknown as { pagination: { total: number } }).pagination?.total || 0,
        });
  ...
  const statCards: StatCard[] = [
    { label: 'Total Orders',    value: stats.totalOrders,                  icon: <ShoppingBag size={22} className="text-blue-600" />,   bg: 'bg-blue-50'   },
    { label: 'Revenue',         value: `₹${stats.totalRevenue.toFixed(2)}`,icon: <TrendingUp  size={22} className="text-green-600" />,  bg: 'bg-green-50'  },
    { label: 'Pending Orders',  value: stats.pendingOrders,                icon: <Package     size={22} className="text-yellow-600" />, bg: 'bg-yellow-50' },
    { label: 'Products',        value: stats.totalProducts,                icon: <Users       size={22} className="text-purple-600" />, bg: 'bg-purple-50' },
  ];
