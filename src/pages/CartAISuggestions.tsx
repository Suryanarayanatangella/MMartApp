...
interface CartAISuggestionsProps {
  suggestions: CartAISuggestionsData | null;
  loading: boolean;
  onRefresh: () => void;
}
...
export default function CartAISuggestions({
  suggestions,
  loading,
  onRefresh,
}: CartAISuggestionsProps) {
                ...
                <div className="flex gap-2 flex-wrap">
                  {b.products.map((p) => (
                    <div key={p.id} className="flex items-center gap-1.5 bg-white rounded-lg px-2 py-1 border border-gray-100">
                      <img
                        src={p.image || 'https://placehold.co/24x24'}
                        alt={p.name}
                        className="w-6 h-6 object-cover rounded"
                        loading="lazy"
                        onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/24x24'; }}
                      />
